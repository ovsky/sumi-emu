...
class ThermalManager(
   ...
   private var currentTemperature = 0
    ...
    public fun startThermalMonitoring() {
    ...
    public fun stopThermalMonitoring()
    {
    ...
    private fun slowDownAllThreads() {
        ...
        while (isThrottling)
        {
            for (t in threads) {
                try {
                   if (t != null && t.isAlive && ((t as Thread) != Looper.getMainLooper().thread)) {
                    // if (t != null && t.isAlive) {
...
// UI Alerts
    ...
    private fun showGameNameAlert(gameName: String) {
        (context as? Activity)?.runOnUiThread {
            AlertDialog.Builder(context)
                .setTitle(gameName)
                .setMessage("Started name: $gameName")
...
// Companion Methods - GPU Speed Limit
    ...
    //     @JvmStatic
        ...
        public fun SetGpuSpeedLimit(value: Int, useLimiter : Boolean = true, context: Context) {
            ...
            showGameNameAlert(game.programId)
            ...
            if (!NativeConfig.isPerGameConfigLoaded()){
                gameConfig = NativeConfig.initializePerGameConfig(game.programId, game.path);
            }

            if (NativeConfig.isPerGameConfigLoaded()){

                if ((gameConfig as NativeConfig) != null) {
                    gameConfig.setBoolean("use_speed_limit", true)
                    gameConfig.setShort("speed_limit", value.toShort())
//
//                    ShortSetting.RENDERER_SPEED_LIMIT.setShort(value.toShort())
//                    BooleanSetting.RENDERER_USE_SPEED_LIMIT.setBoolean(true)
//                    NativeConfig.setBoolean("use_speed_limit", true)
//                    NativeConfig.setShort("speed_limit", value.toShort())

                    gameConfig.savePerGameConfig()
                    gameConfig.reloadGlobalConfig()
                }
            }else
            {
                ShortSetting.RENDERER_SPEED_LIMIT.setShort(value.toShort())
