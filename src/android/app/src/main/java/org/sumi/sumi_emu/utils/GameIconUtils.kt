...
object GameIconUtils {
    private val imageLoader = ImageLoader.Builder(SumiApplication.appContext)
        .components {
            add(GameIconKeyer())
            add(GameIconFetcher.Factory())
        }
        .memoryCache {
            MemoryCache.Builder(SumiApplication.appContext)
                .maxSizePercent(0.25)
                .build()
        }
        .build()
    ...
    fun loadGameIcon(game: Game, imageView: ImageView) {
        val request = ImageRequest.Builder(SumiApplication.appContext)
            .data(game)
            .target(imageView)
    ...
    suspend fun getShortcutIcon(lifecycleOwner: LifecycleOwner, game: Game): IconCompat {
        val layerDrawable = ResourcesCompat.getDrawable(
            SumiApplication.appContext.resources,
            R.drawable.shortcut,
            null
        ) as LayerDrawable
        layerDrawable.setDrawableByLayerId(
            R.id.shortcut_foreground,
            getGameIcon(lifecycleOwner, game).toDrawable(SumiApplication.appContext.resources)
        )
        val inset = SumiApplication.appContext.resources
