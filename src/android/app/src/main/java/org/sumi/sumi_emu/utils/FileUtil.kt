...
import org.sumi.sumi_emu.model.MinimalDocumentFile
import org.sumi.sumi_emu.model.TaskState
...
object FileUtil {
    ...
    fun getFilename(uri: Uri): String {
        val resolver = SumiApplication.appContext.contentResolver
        val columns = arrayOf(
            DocumentsContract.Document.COLUMN_DISPLAY_NAME
        )
        var filename = ""
    ...
    fun DocumentFile.copyFilesTo(
        ...
        this.listFiles().forEach {
            ...
            if (it.isDirectory) {
                newFile.mkdirs()
                DocumentFile.fromTreeUri(SumiApplication.appContext, it.uri)?.copyFilesTo(newFile)
            } else {
                val inputStream =
                    SumiApplication.appContext.contentResolver.openInputStream(it.uri)
    ...
    fun DocumentFile.inputStream(): InputStream =
        SumiApplication.appContext.contentResolver.openInputStream(uri)!!

    fun DocumentFile.outputStream(): OutputStream =
        SumiApplication.appContext.contentResolver.openOutputStream(uri)!!

    fun Uri.inputStream(): InputStream =
        SumiApplication.appContext.contentResolver.openInputStream(this)!!

    fun Uri.outputStream(): OutputStream =
        SumiApplication.appContext.contentResolver.openOutputStream(this)!!

    fun Uri.asDocumentFile(): DocumentFile? =
        DocumentFile.fromSingleUri(SumiApplication.appContext, this)
}
