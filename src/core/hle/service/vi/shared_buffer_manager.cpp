...
Result AllocateSharedBufferMemory(std::unique_ptr<Kernel::KPageGroup>* out_page_group,
    ...
    using Core::Memory::SUMI_PAGESIZE;
...
Result MapSharedBufferIntoProcessAddressSpace(Common::ProcessAddress* out_map_address,
    ...
    const VAddr alias_code_size = page_table.GetAliasCodeRegionSize() / SUMI_PAGESIZE;
    ...
    int i;
    for (i = 0; i < 64; i++) {
        *out_map_address = alias_code_begin + ((rng() % alias_code_size) * SUMI_PAGESIZE);
