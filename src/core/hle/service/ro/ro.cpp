...
struct ProcessContext {
    ...
    Result ValidateNro(ModuleId* out_module_id, u64* out_rx_size, u64* out_ro_size,
        ...
        R_UNLESS(Common::IsAligned(text_size, Core::Memory::SUMI_PAGESIZE), RO::ResultInvalidNro);
        R_UNLESS(Common::IsAligned(ro_size, Core::Memory::SUMI_PAGESIZE), RO::ResultInvalidNro);
        R_UNLESS(Common::IsAligned(rw_size, Core::Memory::SUMI_PAGESIZE), RO::ResultInvalidNro);
        R_UNLESS(Common::IsAligned(bss_size, Core::Memory::SUMI_PAGESIZE), RO::ResultInvalidNro);
...
Result ValidateAddressAndNonZeroSize(u64 address, u64 size) {
    R_UNLESS(Common::IsAligned(address, Core::Memory::SUMI_PAGESIZE), RO::ResultInvalidAddress);
    R_UNLESS(size != 0, RO::ResultInvalidSize);
    R_UNLESS(Common::IsAligned(size, Core::Memory::SUMI_PAGESIZE), RO::ResultInvalidSize);
...
public:
    ...
    Result UnregisterModuleInfo(size_t context_id, u64 nrr_address) {
        ...
        // Validate address.
        R_UNLESS(Common::IsAligned(nrr_address, Core::Memory::SUMI_PAGESIZE),
    ...
    Result UnmapManualLoadModuleMemory(size_t context_id, u64 nro_address) {
        ...
        // Validate address.
        R_UNLESS(Common::IsAligned(nro_address, Core::Memory::SUMI_PAGESIZE),
                 RO::ResultInvalidAddress);
