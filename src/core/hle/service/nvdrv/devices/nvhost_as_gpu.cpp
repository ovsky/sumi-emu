...
NvResult nvhost_as_gpu::AllocateSpace(IoctlAllocSpace& params) {
    ...
    if (params.page_size != VM::SUMI_PAGESIZE && params.page_size != vm.big_page_size) {
        return NvResult::BadValue;
    }
    ...
    const u32 page_size_bits{params.page_size == VM::SUMI_PAGESIZE ? VM::PAGE_SIZE_BITS
                                                                   : vm.big_page_size_bits};

    auto& allocator{params.page_size == VM::SUMI_PAGESIZE ? *vm.small_page_allocator
    ...
    allocation_map[params.offset] = {
        ...
        .big_pages = params.page_size != VM::SUMI_PAGESIZE,
    };
...
void nvhost_as_gpu::FreeMappingLocked(u64 offset) {
    ...
    if (!mapping->fixed) {
        ...
        u32 page_size{mapping->big_page ? vm.big_page_size : VM::SUMI_PAGESIZE};
...
NvResult nvhost_as_gpu::FreeSpace(IoctlFreeSpace& params) {
    ...
    try {
        ...
        auto& allocator{params.page_size == VM::SUMI_PAGESIZE ? *vm.small_page_allocator
                                                              : *vm.big_page_allocator};
        u32 page_size_bits{params.page_size == VM::SUMI_PAGESIZE ? VM::PAGE_SIZE_BITS
                                                                 : vm.big_page_size_bits};
...
NvResult nvhost_as_gpu::MapBufferEx(IoctlMapBufferEx& params) {
    ...
    bool big_page{[&]() {
        if (Common::IsAligned(handle->align, vm.big_page_size)) {
            return true;
        } else if (Common::IsAligned(handle->align, VM::SUMI_PAGESIZE)) {
            return false;
        } else {
            ASSERT(false);
            return false;
    ...
    if ((params.flags & MappingFlags::Fixed) != MappingFlags::None) {
        ...
        u32 page_size{big_page ? vm.big_page_size : VM::SUMI_PAGESIZE};
