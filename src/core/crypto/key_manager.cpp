...
std::optional<Key128> DeriveSDSeed() {
    const auto system_save_43_path =
        Common::FS::GetSumiPath(Common::FS::SumiPath::NANDDir) / "system/save/8000000000000043";
    ...
    const auto sd_private_path =
        Common::FS::GetSumiPath(Common::FS::SumiPath::SDMCDir) / "Nintendo/Contents/private";
...
void KeyManager::ReloadKeys() {
    ...
    if (!Common::FS::CreateDir(sumi_keys_dir)) {
        LOG_ERROR(Core, "Failed to create the keys directory.");
    }

    if (Settings::values.use_dev_keys) {
        dev_mode = true;
        LoadFromFile(sumi_keys_dir / "dev.keys_autogenerated", false);
        LoadFromFile(sumi_keys_dir / "dev.keys", false);
    } else {
        dev_mode = false;
        LoadFromFile(sumi_keys_dir / "prod.keys_autogenerated", false);
        LoadFromFile(sumi_keys_dir / "prod.keys", false);
    }

    LoadFromFile(sumi_keys_dir / "title.keys_autogenerated", true);
    LoadFromFile(sumi_keys_dir / "title.keys", true);
    LoadFromFile(sumi_keys_dir / "console.keys_autogenerated", false);
    LoadFromFile(sumi_keys_dir / "console.keys", false);
}
...
void KeyManager::WriteKeyToFile(KeyCategory category, std::string_view keyname,
    ...
    const auto path = sumi_keys_dir / filename;
    ...
    if (add_info_text) {
        void(file.WriteString(
            "# This file is autogenerated by Sumi\n"
...
bool KeyManager::KeyFileExists(bool title) {
    ...
    if (title) {
        return Common::FS::Exists(sumi_keys_dir / "title.keys");
    }

    if (Settings::values.use_dev_keys) {
        return Common::FS::Exists(sumi_keys_dir / "dev.keys");
    }

    return Common::FS::Exists(sumi_keys_dir / "prod.keys");
}
...
void KeyManager::PopulateTickets() {
    ...
    const auto system_save_e1_path =
        Common::FS::GetSumiPath(Common::FS::SumiPath::NANDDir) / "system/save/80000000000000e1";
    ...
    const auto system_save_e2_path =
        Common::FS::GetSumiPath(Common::FS::SumiPath::NANDDir) / "system/save/80000000000000e2";
