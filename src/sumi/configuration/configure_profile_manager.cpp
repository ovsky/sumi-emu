...
#include "sumi/configuration/configure_profile_manager.h"
#include "sumi/util/limitable_input_dialog.h"
...
QString GetImagePath(const Common::UUID& uuid) {
    const auto path =
        Common::FS::GetSumiPath(Common::FS::SumiPath::NANDDir) /
...
void ConfigureProfileManager::UpdateCurrentUser() {
    ...
    // Set the username for the Sumi profile
    Settings::values.sumi_username = username.toStdString();
}
...
void ConfigureProfileManager::SetUserImage() {
    ...
    const auto raw_path = QString::fromStdString(Common::FS::PathToUTF8String(
        Common::FS::GetSumiPath(Common::FS::SumiPath::NANDDir) / "system/save/8000000000000010"));
