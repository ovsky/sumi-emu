...
#include "sumi/main.h"
#include "sumi/multiplayer/host_room.h"
#include "sumi/multiplayer/message.h"
#include "sumi/multiplayer/state.h"
#include "sumi/multiplayer/validation.h"
...
HostRoomWindow::HostRoomWindow(QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->username->text().isEmpty() && !Settings::values.sumi_username.GetValue().empty()) {
        // Use sumi Web Service user name as nickname by default
        ui->username->setText(QString::fromStdString(Settings::values.sumi_username.GetValue()));
    }
    ...
    if (auto member = room_network.GetRoomMember().lock()) {
        ...
        if (auto room = room_network.GetRoom().lock()) {
            const bool created =
                room->Create(ui->room_name->text().toStdString(),
                             ...
                             ui->max_player->value(), Settings::values.sumi_username.GetValue(),
        ...
        if (is_public) {
            if (auto session = announce_multiplayer_session.lock()) {
                ...
                if (result.result_code != WebService::WebResult::Code::Success) {
                    QMessageBox::warning(
                        this, tr("Error"),
                        tr("To host a room publicly, you must have a valid sumi account configured in "
        ...
        if (is_public) {
            WebService::Client client(Settings::values.web_api_url.GetValue(),
                                      Settings::values.sumi_username.GetValue(),
                                      Settings::values.sumi_token.GetValue());
            if (auto room = room_network.GetRoom().lock()) {
