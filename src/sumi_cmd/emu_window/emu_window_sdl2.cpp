...
void EmuWindow_SDL2::WaitEvent() {
    ...
    if (current_time > last_time + 2000) {
        ...
        const auto title =
            fmt::format("sumi {} | {}-{} | FPS: {:.0f} ({:.0f}%)", Common::g_build_fullname,
...
void EmuWindow_SDL2::SetWindowIcon() {
    SDL_RWops* const sumi_icon_stream = SDL_RWFromConstMem((void*)sumi_icon, sumi_icon_size);
    if (sumi_icon_stream == nullptr) {
        LOG_WARNING(Frontend, "Failed to create sumi icon stream.");
        return;
    }
    SDL_Surface* const window_icon = SDL_LoadBMP_RW(sumi_icon_stream, 1);
