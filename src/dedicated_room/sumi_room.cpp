...
static void PrintHelp(const char* argv0) {
    LOG_INFO(Network,
             ...
             "--web-api-url       sumi Web API url\n"
             "--ban-list-file     The file for storing the room ban list\n"
             "--log-file          The file for storing the room log\n"
             "--enable-sumi-mods Allow sumi Community Moderators to moderate on your room\n"
...
static void PrintVersion() {
    LOG_INFO(Network, "sumi dedicated room {} {} Libnetwork: {}", Common::g_scm_branch,
             Common::g_scm_desc, Network::network_version);
}

/// The magic text at the beginning of a sumi-room ban list file.
static constexpr char BanListMagic[] = "SumiRoom-BanList-1";
...
int main(int argc, char** argv) {
    ...
    std::string log_file = "sumi-room.log";
    ...
    static struct option long_options[] = {
        ...
        {"enable-sumi-mods", no_argument, 0, 'e'},
    ...
    if (announce) {
        if (username.empty()) {
            ...
            PadToken(token);
            Settings::values.sumi_username = UsernameFromDisplayToken(token);
            username = Settings::values.sumi_username.GetValue();
            Settings::values.sumi_token = TokenFromDisplayToken(token);
        } else {
            LOG_INFO(Network, "Hosting a public room");
            Settings::values.web_api_url = web_api_url;
            Settings::values.sumi_username = username;
            Settings::values.sumi_token = token;
        }
    }
...
#else
        LOG_INFO(Network,
                 "sumi Web Services is not available with this build: validation is disabled.");
