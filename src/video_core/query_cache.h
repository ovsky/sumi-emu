...
private:
    ...
    void FlushAndRemoveRegion(VAddr addr, std::size_t size, bool async = false) {
        ...
        const u64 page_end = addr_end >> SUMI_PAGEBITS;
        for (u64 page = addr_begin >> SUMI_PAGEBITS; page <= page_end; ++page) {
    ...
    CachedQuery* Register(VideoCore::QueryType type, VAddr cpu_addr, u8* host_ptr, bool timestamp) {
        const u64 page = static_cast<u64>(cpu_addr) >> SUMI_PAGEBITS;
    ...
    CachedQuery* TryGet(VAddr addr) {
        const u64 page = static_cast<u64>(addr) >> SUMI_PAGEBITS;
        const auto it = cached_queries.find(page);
        if (it == std::end(cached_queries)) {
            return nullptr;
        }
    ...
    static constexpr std::uintptr_t SUMI_PAGESIZE = 4096;
    static constexpr unsigned SUMI_PAGEBITS = 12;
