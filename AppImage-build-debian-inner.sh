...
test -e /tmp/sumi-src || (echo "Script MUST NOT be called directly!" ; exit 1)
...
mkdir sumi-build
cd sumi-build
cmake /tmp/sumi-src -GNinja -DCMAKE_BUILD_TYPE=Release -DSUMI_TESTS=OFF -DENABLE_QT_TRANSLATION=OFF -DSPIRV_WERROR=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_FIND_LIBRARY_SUFFIXES=".a;.so" -DSPIRV-Headers_SOURCE_DIR=/tmp/sumi-src/externals/SPIRV-Headers
ninja

# Generate AppImage
cp -rv /tmp/sumi-src/AppImageBuilder /tmp/AppImageBuilder
cd /tmp/AppImageBuilder
./build.sh /tmp/sumi-build /tmp/sumi.AppImage || echo "This error is known. Using workaround..."
cp /lib/$(uname -m)-linux-gnu/libICE.so.6 build/
mv build /tmp/hosttmp/sumi-debian-appimage-rootfs
